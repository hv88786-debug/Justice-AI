...
6. Check karein: `https://medscan-ai-backend.vercel.app/api/health` khol ke dekhein —
   `{"success": true, ...}` aana chahiye. (Root URL `/` pe 404 aana normal hai — is backend
   mein koi route "/" par define nahi hai, sirf `/api/...` routes hain.)
7. Agar OCR (image reports) processing timeout ho, Vercel dashboard → Project → Settings →
   Functions mein function duration badhayein (Hobby plan par ye limited hota hai; zaroorat
   pade to Fluid Compute enable karein ya Pro plan lein).
...
> Note: `index.html` bhi add kar diya gaya hai (same content as `medscan-ai.html`) —
> Vercel static hosting root URL (`/`) par by default `index.html` dhoondta hai, isliye
> ye zaroori tha (nahi to root pe 404 aata).

1. `medscan-ai.html` **aur** `index.html` — dono mein `API_BASE` ko backend ke actual
   URL se update karein:
   ```js

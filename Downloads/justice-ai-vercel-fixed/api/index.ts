...
// NOTE: On Vercel serverless, each invocation may run in a fresh container,
// so this cache is best-effort (works during warm invocations) and not a
...
// Vercel's Node runtime invokes this exported handler directly for every
// request under /api/*. No app.listen() here — that's only for local dev
// (see server.ts, run via `npm run dev`).
export default app;
